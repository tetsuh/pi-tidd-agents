...
For exact `/tidd-pr ... autofix`, `tidd-autofix-worker` is mandatory: it is the sole correction writer and publisher, never merely a default. Its launch request is built by `build_writer_launch` and passed to the subagent tool unchanged: the parent supplies the created workspace and the task, the builder the rest (CL-D81). The package ships no other worker: a gate verdict here is an automated exit condition rather than advice to a human, so a model must not grade its own fixes, and the shipped writer default **keeps the closed-loop requirement inside one model family**.
...
A normal commit follows CL-D25: a Conventional Commits subject, issue-number reference, and test provenance in the body. The message is real UTF-8 newline bytes, passed on stdin via `git commit -F - --cleanup=whitespace`; never encode literal `\\n` sequences. The writer makes it through packaged `commit_create`, which carries the operator identity `operator_capture` recorded at preflight; a checkout without a `user.name` and `user.email` stops there, before any gate (CL-D89).
