...
verify_pr_identity() {
  ...
  identity="$(gh api "repos/$REVIEW_REPOSITORY/pulls/$REVIEW_PR_NUMBER" --jq 'if any([.base.sha, .head.repo.full_name, .head.ref][]; . != null and (type == "object" or type == "array")) then error("identity field has invalid type") else [.base.repo.full_name, (.number|tostring), .state, (.draft|tostring), .head.sha, .html_url, .base.sha, (.head.repo.full_name // ""), .head.ref] | map(. // "" | tostring) | join("\u001f") end' 2>"$scratch/identity-$phase.err")" || fail "pull-request identity lookup failed at $phase"
