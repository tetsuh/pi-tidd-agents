...
const FINDING_ID = /^[A-Z][A-Z0-9]*-\d+-[A-Za-z0-9-]+$/;
const DISPOSITION = /^[a-z][a-z-]*$/;
...
function checkBinding(binding) {
  ...
  if (!text(binding.sourceKind) || !KIND.test(binding.sourceKind)) fail('invalid_reply_binding', 'sourceKind must be a lowercase token');
  ...
  if (!text(binding.sourceUrl) || !/^https:\/\/github\.com\/\S+$/.test(binding.sourceUrl)) fail('invalid_reply_binding', 'sourceUrl must be a github.com URL without spaces');
  ...
  for (const finding of binding.findings) {
    ...
    if (!text(finding.disposition) || !DISPOSITION.test(finding.disposition)) fail('invalid_reply_binding', 'disposition must be a lowercase token');
...
function createReplyMarker({ binding, visibleBody } = {}) {
  try {
    ...
    const visible = canonicalVisible(visibleBody);
...
function classify({ binding, visibleSha256, source, comments, paginationComplete, currentHead }) {
  ...
  for (const comment of comments) {
    if (!plain(comment) || typeof comment.body !== 'string') return ambiguous('destination_evidence_missing');
