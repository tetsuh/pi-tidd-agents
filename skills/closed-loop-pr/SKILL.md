...
- `CLEAN@H` := ordinary operational cleanliness at exact public head `H`:
  ...
  4. no untracked path exists outside the exact repository-root `.pi` or `.pi/` lexical namespace;
  5. the repository-root `.pi` is absent or a real directory inspected without following links; a symlink, regular file, FIFO, socket, device, unknown kind, or followed root fails closed, while descendants are classified lexically without following targets; and
  6. neither the candidate/public `H` tree nor the index contains `.pi` or `.pi/**` in any mode or stage. `.pi2`, `x/.pi`, and `foo.pi` remain outside the namespace. Tracked blobs, symlinks, gitlinks, intent-to-add, staged add/modify/rename/delete, and conflict stages fail closed; a staged deletion never cures a forbidden parent entry. Only untracked descendants below a verified real root receive the condition-4 exception.
- `POST_COMMIT(C, P)` := complete CL-D30 identity remains unchanged except that local `HEAD=C` while remote/public head remains `P`; `C` has sole parent `P`; conditions 2–6 of `CLEAN@H` hold as specialized to local commit `C` and public parent `P`; tracked worktree, index, and `unstaged` are clean; the immutable manifest still matches its independently captured index state and commit `C` exactly by parent, tree, path/status/mode inventory, and blob identity; and no forbidden `.pi` entry exists in `P`, `C`, the index, manifest, manifest tree/inventory, commit tree/inventory, or published candidate state. It deliberately does not require local and public head equality.
...
Before its first edit and immediately before commit, Luna rejects every `.pi` and `.pi/**` authorized correction path and re-resolves the complete identity; every intervening guard does the same. After editing, Luna freezes the exact authorized non-`.pi` overlay by raw diff/content bytes, path, status, mode, and blob identity.
...
- `AFTER_VALIDATION` immediately after focused and required publication validation requires `CLEAN@P` conditions 1 and 3–6; condition 2 is replaced only by byte-for-byte and path/status/mode/blob equality with the validated frozen overlay. It rejects unauthorized tracked paths, index change, outside untracked dirt, unsafe root, `.pi` authority, and validation drift.
- `BEFORE_STAGING` immediately before staging repeats `CLEAN@P` conditions 1 and 3–6 with the same condition-2 validated-overlay replacement and freezes that overlay unchanged. Runtime `.pi/**` churn remains only untracked state below the safe real root and never enters the overlay.
...
- `AFTER_STAGING` immediately after staging requires `CLEAN@P` conditions 1 and 4–6; conditions 2 and 3 are replaced by no unstaged candidate change and an index exactly equal to the immutable manifest and independently captured index state. Manifest parent/tree/inventory/blob identities must match; every `.pi` path in `P`, the index, manifest tree/inventory, or staged inventory fails closed regardless of its mode, stage, intent-to-add, or add/modify/rename/delete/conflict status. A staged deletion never cures a forbidden entry in `P`. The index is intentionally not clean only because it contains exactly the immutable manifest.
