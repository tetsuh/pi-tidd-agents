...
Every formal Sol and Terra invocation in both workflow roots requests the packaged closed result schema through its `outputSchema` and returns that structured envelope. The validated envelope is the parent's sole verdict, correlation, finding, confirmation, and decision authority; never parse a verdict, correlation field, or finding record out of Markdown. The human-readable report is generated from the envelope and still ends with the required final-line verdict token, which remains `MERGE | FIX BEFORE MERGE | NEEDS DECISION`. The parent supplies its own assigned finding set and a gate-scoped fresh finding namespace. Assigned findings require exactly one gate/head-bound confirmation; a result that omits one, falsely labels an unassigned ID as assigned, duplicates an ID, or confirms another gate/head fails closed. Fresh findings use that namespace without a confirmation or `blockerKey`; after intake the parent assigns `blockerKey`, breaker owner, and confirmation routing before any successor action. The parent also supplies the exact required evidence-attestation set; omissions, extras, duplicates, and incomplete reads fail closed, and Sol must return at least one complete adversarial-result record. This separation lets every gate report newly discovered CL-D29 counterexamples without weakening assigned-set checks or CL-D2/CL-D29 duties. A structured-output startup, schema-validation, or transport failure is a tool failure that consumes no counter and is never a verdict.
