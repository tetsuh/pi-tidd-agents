...
Every formal Sol and Terra invocation in both workflow roots requests the packaged closed result schema through its `outputSchema` and returns that structured envelope. After runtime schema validation, every Issue, PR review-only, and exact-autofix route passes the envelope plus parent expectations through packaged `gate_result_validate` before reading any field; `outputSchema` alone is insufficient because it cannot enforce cross-record semantics. Only that validated envelope is the parent's verdict, correlation, finding, confirmation, and decision authority; never parse those fields from Markdown. The generated human report still ends with the final-line token `MERGE | FIX BEFORE MERGE | NEEDS DECISION`.

The parent supplies workflow kind, complete assigned `{findingId, blockerKey}` tuples, and the exact required evidence-attestation set; the fresh finding namespace is derived, never supplied (CL-D47), and every `file`-kind entry of that set must exist, verified through packaged `required_evidence_check` before the expectation is built, so a required file that does not exist is an assembly error and never a gate outcome (CL-D61). Assigned findings require exactly one gate/head-bound confirmation and the exact parent tuple; omission, collision, substitution, duplicate, stale gate/head, evidence omission/extra/duplicate/incomplete, and incomplete root-specific finding records fail closed. Issue records carry only candidate identity, fixed revised passage, and snapshot assignment. PR records select `gate | body | issue-comment | review | inline-comment | check | status`: non-gate variants require source URL, body digest, and timestamps; review adds review-commit identity; inline-comment adds review-commit, path, and line identity; incompatible variant fields fail closed.

Fresh findings use the namespace without confirmation or `blockerKey`; after validated intake the parent assigns `blockerKey`, breaker owner, and confirmation routing before any successor action. Sol returns at least one complete adversarial-result record; each counterexample or unavailable-evidence result links to its finding, and material evidence cannot simultaneously confirm that finding as fixed. This separation permits every newly discovered CL-D29 counterexample without weakening CL-D34's explicitly non-blocking follow-up/out-of-scope classes, assigned-set checks, CL-D2/CL-D29, or workflow-root record duties. A structured-output startup, schema-validation, semantic-validation, or transport failure is a tool failure that consumes no counter and is never a verdict.

Designated output (CL-D58): the gate envelope is the file at the run's structured output path, as reported by the runner's run status; the parent reads it from that path after completion. A completion notice, wait result, or preview is a courier and never the envelope: `(no output)` in one is transport indeterminacy, not zero output, and the parent must read the path before classifying the run. Zero designated output bytes (CL-D51) means that file is absent or empty after completion; an exit code, success flag, or runner status never substitutes for reading it. In either direction: a validated envelope at that path is the verdict whatever the runner's status says; a validation failure at that path remains a tool failure governed by the owning workflow or selected mode's existing retry/fail-stop rule. This route-neutral rule grants no retry change: exact autofix's first-malformed-result terminal rule and CL-D51's zero-byte relaunch remain unchanged.

Gate identities (CL-D60): under envelope schema version 2 the gate is `adversarial` (the Sol gate on both roots), `decision-drift` (the Terra gate on the Issue root), or `safety` (the Terra gate on the PR root), or `convergence` (the non-authoritative preliminary stage on both roots, CL-D62); a version 2 envelope naming a gate outside its root fails closed, and the derived fresh-finding namespaces are `ADV-<n>-`, `DRIFT-<n>-`, and `SAFETY-<n>-` (`CONV-<n>-` for convergence). Version 1 (`sol`, `terra`, `SOL-`, `TERRA-`) remains accepted for one release by an explicit version branch with no cross-mapping between versions; the packaged expectation builder ships version 2 only, and a publication marker names its gates in the vocabulary of the envelope it was written from, and both vocabularies parse for that release.
...
`tidd-convergence-reviewer` is a read-only, fresh-context, non-authoritative preliminary reviewer that runs once per candidate identity and snapshot fingerprint before the adversarial gate on both roots: Issue `convergence → adversarial → decision-drift`, PR `convergence → adversarial → safety`. It returns the CL-D36 envelope with gate `convergence` and the derived namespace `CONV-<n>-`; its findings enter the same disposition model and reach the adversarial gate as assigned findings, and no convergence outcome can declare `IMPLEMENTATION_READY` or `MERGE_READY`: readiness still requires the unchanged formal gates. A candidate, body, diff, ledger, or snapshot change invalidates convergence results exactly as it invalidates formal results, and the sequence restarts at convergence; new snapshot evidence on an unchanged head reruns convergence before Sol within the same cap (DEC-109-CONV-SNAPSHOT-001). Rounds are accounted separately as `convergence <used>/<cap>`, never against a formal gate's budget: the cap is 3 per run on the Issue root and PR review-only and 5 on exact autofix; tool, provider, startup, and unparsable-verdict failures do not consume it, and the owning route's retry rule applies. When the cap is reached with findings open, the parent hands the current candidate to the adversarial gate with those findings assigned and does not invoke convergence again; `ROUND_LIMIT_REACHED` remains reserved for the formal gates. A `NEEDS DECISION` verdict from convergence takes the ordinary owner-decision path. If the role is disabled through pi-subagents configuration the stage is skipped and the status block reports `convergence: disabled`; a role that is present but unresolved or resolves with `edit` or `write` is `BLOCKED` under the shared rule. Every status block reports `resolved:` with the provider, model, and thinking level each role ran with, as reported by the runner, and the counts of convergence and adversarial invocations. The convergence stage requests the same packaged schema through its `outputSchema` and composes the Every-gate invariant payload block and the volatile envelope, never the Sol-only adversarial block or a formal role-authority block. The no-progress rule is shared: a convergence result's unresolved observation of an assigned `blockerKey × breakerOwner` counts toward that key's history exactly as a formal result's does, at most one per result, and the third observation across convergence and formal gates stops the run (`ROUND_LIMIT_REACHED(reason=no_progress)` on exact autofix), while convergence stays outside the formal invocation budget.
