...
Every `Blocker` or `Major` finding names the acceptance criterion, contract clause, or fail-stop invariant its counterexample falsifies. A claim that appears only in target-body prose and traces to no criterion or clause is a `reword` finding: its correction is an owner edit of the body, never an implementation blocker. A counterexample that requires violating an assumed operator condition declared cooperative by the selected mode's threat model is a `follow-up` finding reported with a proposed issue title, unless an acceptance criterion names that condition; it is not a blocker. Bounding applies to severity and disposition only: Sol still reads implementation and tests completely and still reports every cited counterexample.
