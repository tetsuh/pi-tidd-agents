...
*Target and revision:* `tetsuh/pi-tidd-agents#74` at its initial body, which carries the observed reproducer from the authoritative Issue #36 comment, and the owner's live instruction `#74 を進めて`
...
*Owner choice:* Option C. `envelope`, `data`, and `receipt` are distinct input shapes, decided by structure rather than by what a caller names them. Each cross-operation field declares exactly one, the map names it beside the field, and a supplied shape that is not the declared one stops with `input_shape_mismatch` before the operation runs, naming the field, the declared shape, and what arrived. The packaged CLI accepts no tolerant alternative for a declared field; direct helper calls retain their base-compatible argument and output semantics. The check is expressed as one predicate per declared shape: a supplied value is rejected because it fails the declared field's predicate, never because of what it was recognized to be, and no total classification of arbitrary objects is part of the contract. Predicate depth is calibrated to what fails downstream — `snapshot` carries the full producer shape because `fingerprint_snapshot` hashes any object silently, while `result` is only a separating probe because the closed CL-D36 schema remains the sole validator of gate results. The five declared fields are the complete CL-D44 set.
