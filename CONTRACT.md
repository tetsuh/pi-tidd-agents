...
Autofix requires the head branch already checked out and ordinary operational cleanliness: the complete target/checkout identity and local `HEAD` equal the expected exact public head; the tracked worktree and index have no changes; no untracked path exists outside the repository-root `.pi/**` namespace; the `.pi` root is absent or a real directory inspected without following links; and neither the candidate/public `HEAD` tree nor the index contains `.pi` or `.pi/**` in any mode or stage. The namespace is exactly `.pi` and descendants beginning `.pi/`; `.pi2`, `x/.pi`, and `foo.pi` are outside it. A root symlink, regular file, FIFO, socket, device, or unknown kind fails closed, and descendant symlinks are classified lexically without following their targets. Unchanged tracked entries, blobs, symlinks, gitlinks, intent-to-add entries, staged additions, modifications, deletions, renames, and unmerged/conflict stages are candidate state and fail closed; a staged deletion cannot cure a forbidden path in the candidate parent. The complete `.pi/**` exception applies only to untracked runtime descendants below a verified real root. It grants no cleanup, ignore, stash, discard, retry, resume, or mutation authority. The workflow never switches branches, stashes, or discards: that is a git-state change nobody authorised and it can destroy uncommitted work.
...
**Issue #17 operational-cleanliness amendment:** At every ordinary gate, reply, final-classification, and post-push boundary, runtime bytes below a verified real repository-root `.pi/` directory are excluded only from untracked cleanliness; the root must be a real directory or absent and is inspected without following links; root safety, exact identity, tracked worktree, index, and candidate/public `HEAD` checks remain mandatory. During correction/validation, only the exact authorized overlay is permitted; before commit, the immutable manifest must match the index and contain no `.pi` path; after commit and before push, use distinct `local HEAD=C`, sole parent `P`, remote/public `P` guards with clean tracked worktree/index, exact manifest/tree/inventory/blob identity, and no `.pi` entry in parent, index, manifest, commit, or published candidate tree. The AFTER_COMMIT and repeated BEFORE_PUSH guards independently reclassify the root without following links. Between them, safe untracked runtime churn below that root may change, including descendant create, content change, rename, removal, and a safe root transition between absent and real directory, while candidate, tracked worktree/index/unstaged, manifest, commit, evidence, and public-head identity remain unchanged; every other change fails closed before push. Base-only `.pi` deletion remains visible in the unfiltered raw effective diff and is never correction scope. Runtime churn contributes no candidate, evidence, or publication bytes and must not be claimed as cleaned, preserved, validated, committed, or published. Outside untracked dirt, unsafe roots, forbidden Git entries, identity movement, validation/manifest/commit failure, retry, resume, cleanup, compensation, or any other existing CL-D30 failure remains fail-closed with no effect.
