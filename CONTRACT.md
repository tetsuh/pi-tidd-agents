...
## CL-D41 — Exact autofix bounds one pull request across fresh runs
**Clauses:** CL-D41-autofix, CL-D41-helper, CL-D41-readme, CL-D41-tests

*Decision ID:* CL-D41
*Kind:* contract
*Target and revision:* `tetsuh/pi-tidd-agents#50` at the current Issue #50 body and the owner's live instruction `#50 を進めて`
*Question:* Can exact PR `autofix` bound the sequence of fresh runs against one pull request without introducing durable workflow state, and how much of Issue #50 is reachable that way?
*Options and trade-offs:* Option A keeps per-run counters only and leaves the unbounded run sequence Issue #50 reports. Option B stores counters and the settled ledger per pull request, which is what the issue proposes but which CL-D30 already excluded when it recorded that the addendum adds no durable workflow state, and which the shared base accounting separately forbids as a state file. Option C derives the accounting from evidence the target already carries, which bounds the run sequence immediately but reaches only the part that is derivable. A leaves the defect in place; B is the complete answer and contradicts a standing decision; C is incomplete and truthful about being incomplete.
*Recommendation:* Option C now, with the underivable remainder named and deferred rather than approximated.
*Owner choice:* Option C. One pull request is bounded by the accounting key `(repository, pull-request number, base OID)`, derived at each snapshot and never stored, so base movement starts a new key and no state file, outbox, or durable workflow state is created. The packaged `pr_accounting` operation counts the correction commits the branch carries since that base and permits at most 10 per key; before a correction a reached ceiling stops `WAITING_FOR_OWNER(reason=owner_decision_required)` carrying the key and the count, never a silent fresh start, and only the owner may continue. The run-scoped 15 gate invocations, 5 successful correction pushes, and third-observation no-progress stop are unchanged, and identity, scope, and safety failures still precede the ceiling.
*Rationale:* Issue #50's own evidence is that every command is a fresh run with fresh counters, so neither `ROUND_LIMIT_REACHED` reason could fire while PR #48 accumulated 23 commits across 25 Sol invocations; a ceiling of 10 correction commits would have stopped that pull request at its tenth commit and handed the decision to the owner. The derived count is a conservative superset of the run-scoped successful-push counter rather than that counter itself, because every successful correction push adds at least one commit and owner commits are counted too; it is named a correction-commit ceiling for that reason and never under-counts pushes. Deriving rather than storing is what keeps this decision compatible with CL-D30, whose rationale already excluded durable workflow state, and with the shared base accounting, which forbids a state file; nothing in this decision reads or writes state between runs. The remainder of Issue #50 is not approximated here: carrying a gate-invocation count, a settled ledger, or prior `MERGE` verdict heads across runs requires per-round records that only the source-reply marker grammar of Issue #40 can make readable, and Issue #40 in turn depends on Issue #36, so those acceptance criteria stay open under Issue #50 rather than being claimed. Novelty-churn detection is deferred with them, because it is a rule over that same per-round key history. Holding the section, its helper-map row, and its one precedence clause raises the six-file authority baseline from 116,000 to 118,000 bytes: the six files measured 114,563 bytes at `c2ad0db` and the addition measured 1,728 bytes, overrunning the prior ceiling by 291, and Issue #58 already harvested cross-file duplication, so the shortfall is funded by the raise rather than by a trim.
*Validity and invalidation conditions:* Applies to exact PR `autofix` only. It grants no state file, outbox, scheduler, resume, retry, or provider authority, adds no shared-layer authority, and changes no Issue or PR review-only behaviour. The ceiling stops the run for an owner decision and never blocks, forces, or continues on its own. Raising the ceiling above 10, deriving the count from anything other than the branch commits since the recorded base, or claiming any inherited gate-invocation count, settled ledger, prior `MERGE` head, or novelty-churn observation requires a new owner decision. Should the count ever be sourced from records that can under-count pushes, the conservative-superset basis recorded here is invalidated.
