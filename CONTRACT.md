...
*Validity and invalidation conditions:* Applies to the two raised guards only. It weakens no assertion into a warning, trims no prose, and changes no workflow behaviour, agent, or helper. A further raise of either guard, or any raise of the disclosure guard, requires a new owner decision and should be treated as evidence that the split named above is overdue instead. If a duplication harvest later recovers substantial space, the guards should be lowered toward the measurement rather than left wide.
