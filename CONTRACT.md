...
*Owner choice:* Option C. Gate evidence travels in one envelope carrying `schemaVersion`, capture identity, the seven fingerprints, both capture brackets, and completeness. An unknown or absent version fails closed before any field is read, and every section is closed to unknown fields. Under version 1 each fingerprint is a `{ domain, encoding, value }` record whose `domain` must equal the field holding it; `raw_bytes`, `normalized_text`, and `canonical_json` are distinct byte domains, each field fixed to one of them and to one value shape. The packaged `evidence_verify` operation is read-only and repeatable. It compares every one of the seven domains and every identity field — repository, number, base OID, head OID, base branch, head repository, head branch, state, and draft — against the expected target, requires both brackets to witness that same identity, and rejects a moved capture bracket, incomplete pagination or completeness metadata, and an envelope whose head or base fingerprint disagrees with the identity it claims. Lifecycle and branch identity are required rather than nullable, because exact autofix cannot act on a target whose state, draft flag, or head branch it could not observe. The envelope is deliberately narrower than the one Issue #36 sketches: it carries identity, fingerprints, brackets, and completeness, and does **not** carry the review-evidence collections that issue also lists — check runs with app and source identity, commit statuses, reviews and review threads, or issue and review comments and annotations. Those and the cross-operation composition typing recorded in the authoritative Issue #36 comment are deferred, and Issue #36 stays open for them.
*Rationale:* The parent assembles the gate payload and also assembles the expectation it is checked against, so a misplaced value is self-consistent and passes every existing check: `gate_result_validate` compares `requiredEvidence` with `evidenceRead`, and both come from the parent. The three OID operations return the OID they validated, and the three digest operations return a bare 64-hex string over three different byte domains, so nothing in a value records which domain produced it. CL-D39 already recorded this as reachable and its recovery table already carries a row for a digest computed from the wrong domain, which is a documented remedy for a failure nothing detects; the outcome it protects against is a gate reviewing evidence from a different head and returning `MERGE`, which is the one result the closed loop exists to prevent. Labelling makes the value carry its own provenance, so the mix-up is caught without inventing a second source of truth. The bound is exact: it defeats a labelled record copied from the wrong operation, and a value retyped by hand into a well-formed record stays undetectable here, which is why the operation declaring a domain remains its only source. Scope is exact PR `autofix` because that is where six same-shaped values are assembled together; the Issue root carries one specification fingerprint with nothing to confuse it with, and widening the surface without a demonstrated hole was rejected. The section fits under the existing baseline: the six files measure 115,939 bytes, so no ceiling raise is taken.
