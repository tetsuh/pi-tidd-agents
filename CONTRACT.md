...
Autofix requires the head branch already checked out and `CLEAN@H`, the ordinary operational-cleanliness invariant defined under CL-D30, for the expected exact public head `H`. All six conditions apply at preflight: complete target/checkout identity, tracked worktree, index, outside-untracked state, safe no-follow repository-root `.pi` classification, and absence of forbidden `.pi` entries from `H` and the index. The namespace remains exactly `.pi` and descendants beginning `.pi/`; `.pi2`, `x/.pi`, and `foo.pi` remain outside it. A staged deletion never cures a forbidden path in the candidate parent. The invariant grants no cleanup, ignore, stash, discard, retry, resume, or mutation authority. The workflow never switches branches, stashes, or discards: that is a git-state change nobody authorised and it can destroy uncommitted work.
...
- `CLEAN@H` := ordinary operational cleanliness at exact public head `H`: (1) complete CL-D30 target and checkout identity is unchanged, and local `HEAD` and remote/public head equal `H`; (2) tracked worktree has no change; (3) index has no change; (4) no untracked path exists outside repository-root `.pi/**`; (5) the `.pi` root is absent or a real directory inspected without following links, with unsafe root kinds blocked and descendants classified lexically without following targets; and (6) neither candidate/public `H` nor index contains `.pi` or `.pi/**` in any mode or stage. Only untracked descendants below a verified real root receive condition 4's exception. Near names remain outside; every tracked mode/stage and a staged deletion of a forbidden parent entry remain blocked.
- `POST_COMMIT(C, P)` := complete CL-D30 identity remains unchanged except local `HEAD=C` while remote/public head remains `P`; `C` has sole parent `P`; conditions 2–6 of `CLEAN@H` hold as specialized to `C` and `P`; tracked worktree, index, and `unstaged` are clean; immutable manifest, independently captured index state, and `C` match exactly by parent, tree, path/status/mode inventory, and blob identity; and no forbidden `.pi` entry exists in `P`, `C`, index, manifest, manifest tree/inventory, commit tree/inventory, or published candidate state. It deliberately does not require local and public head equality.
