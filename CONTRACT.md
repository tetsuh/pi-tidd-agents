...
## CL-D39 — Exact-autofix tooling failures remain terminal without recovery
**Clauses:** CL-D39-stop, CL-D39-structural

*Decision ID:* CL-D39
*Kind:* contract
*Target and revision:* `tetsuh/pi-tidd-agents#34` at the current Issue #34 body, decided after #47, #55, and #37 merged
*Question:* May exact PR `autofix` recover in the same live run from a bounded, deterministic, pre-mutation local tooling failure, or does every tool failure there stay terminal?
*Options and trade-offs:* Option A retains the universal stop rule; Option B allows one bounded pre-mutation local recovery behind eight re-proved conditions; Option C waits for an executable durable controller to own recovery and state. Option A has the simplest safety argument and discards otherwise valid review evidence when an incidental defect occurs. Option B recovers that evidence at the cost of an exception path through the safety boundary. Option C changes nothing now and carries the largest scope.
*Recommendation:* Option A for exact PR `autofix`, leaving the recovery question open for separate decision rather than settling it here.
*Owner choice:* Option A, scoped to exact PR `autofix`. There, every tool, startup, API, timeout, stale-target, malformed-output, and correlation failure remains a non-verdict that consumes no counter, is not retried, and stops the run, and no bounded local recovery, recovery budget, or prevalidated-replacement path is introduced. Issue and PR review-only keep their existing single malformed-verdict retry unchanged, as Issue #34's non-goals require.
*Rationale:* Exact autofix is the only mode that carries mutation authority, so it is the only mode where a recovery path would sit beside a publication boundary; the other modes' single malformed-verdict retry predates this issue and is out of its scope. Since Issue #34 was filed on 2026-08-10, #47, #55, and #37 have narrowed the surface that produces incidental defects: the packaged helper invocation map forbids regenerating check logic as run-time shell, `jq`, Python, or GraphQL, and only a `gate_result_validate`-checked closed envelope is authoritative, with its fields never parsed from Markdown. That narrowing is real but partial and is not a basis for calling any failure class impossible: a closed envelope rejects unknown fields without preventing selection of the wrong known field, `fingerprint_pr_base`, `fingerprint_pr_tree`, and `fingerprint_pr_head` all accept an `oid` so a same-typed cross-domain mix-up remains reachable, and the staged-manifest comparison is still performed by orchestrator logic with no packaged operation behind it. Issue #34 also records that these failures occurred on a clean unchanged public head with no writer invoked, so at least the validation-harness class is reachable before any edit and a recovery path would not be empty. This decision therefore settles only that exact autofix gains no recovery now; whether it should is reopened separately with that corrected evidence, and the work to remove the remaining classes structurally is tracked in #64.
*Validity and invalidation conditions:* Applies to exact PR `autofix` at every phase and grants no retry, recovery, resume, or mutation authority. It changes no Issue or PR review-only behaviour. The narrowing mechanisms named above are pinned so that weakening the invocation map's no-regeneration rule or the validated-envelope authority fails the build; they support the decision without establishing that any failure class is impossible. Adopting a bounded recovery for exact autofix, or extending this rule to another mode, requires a new owner decision.
