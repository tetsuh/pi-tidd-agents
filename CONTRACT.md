...
## CL-D39 — Pre-mutation tooling failures remain universally terminal
...
*Question:* May a stateless Skill recover in the same live run from a bounded, deterministic, pre-mutation local tooling failure, or does every tool failure stay terminal?
...
*Recommendation:* Option A, because the population of failures Option B was designed to recover has since been removed rather than reduced.
*Owner choice:* Option A. Every tool, startup, API, timeout, stale-target, malformed-output, and correlation failure remains a non-verdict that consumes no counter, is not retried, and stops the run, in every mode and at every phase. No bounded local recovery, recovery budget, or prevalidated-replacement path is introduced.
*Rationale:* Issue #34 recommended Option B on 2026-08-10, before #47, #55, and #37 merged. Measured against the shipped contract, three of the five failure classes it names are now structurally impossible: reading the wrong key from an evidence envelope is prevented because only a `gate_result_validate`-checked closed envelope is authoritative and its fields are never parsed from Markdown; malformed run-time shell or `jq` is prevented because the packaged helper invocation map forbids regenerating that logic at run time; and comparing digests across byte domains is prevented because seven fingerprint operations each declare their own input domain, with the binary diff taking raw bytes. The remaining two are a validation harness error and an over-specific staged-manifest assertion. Both occur after Luna's edit, where `CLEAN@H` no longer holds, so neither is recoverable under Option B's own conditions 1 and 3. Option B would recover approximately nothing while adding an exception path through the safety boundary. The correct response to the survivors is to make the remaining classes impossible rather than recoverable, in the way the other three were, and that work is tracked separately.
*Validity and invalidation conditions:* Applies to every mode and phase and grants no retry, recovery, resume, or mutation authority. It rests on the structural mechanisms named above: if the invocation map's no-regeneration rule, the validated-envelope authority, or the per-domain fingerprint separation is weakened, the basis for Option A is weakened with it and this decision must be revisited before any recovery path is considered instead. Adopting Option B or Option C later requires a new owner decision.
