...
**Clauses:** none — structural. Enforced by `test/package.test.js`, which asserts each prompt names the skill it loads and stays within a line budget.
...
**Clauses:** none — structural. Enforced by `test/package.test.js`, which asserts the test script exists and that there are no `devDependencies`.
...
**Clauses:** none — structural. Enforced by `test/contract-record.test.js`, which is this decision.
...
**Clauses:** none — structural. Enforced by `AC-GRANT` and by the superseded-rule guard in `test/closed-loop-regressions.test.js`, which names the retired publication wordings.
...
**Clauses:** none — structural. Its outcome is enforced by `CL-D24`.
