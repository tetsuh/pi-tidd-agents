...
`terra-oracle` has no verdict contract of its own, and the acceptance criteria require the existing agents to stay unchanged. The verdict line is therefore required through the invocation payload, and nothing under `agents/` is modified. CL-D35 permits only the removal of a tool the runtime does not provide, which reduces capability and cannot move a verdict into an agent file. A missing or unparsable verdict is a tool-level failure: retry once, then report `BLOCKED`.
...
## CL-D35 — Agent tool allowlists may drop a tool the runtime does not provide
