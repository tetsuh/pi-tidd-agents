...
*Owner choice:* Option C. `envelope`, `data`, and `receipt` are distinct input shapes, decided by structure rather than by what a caller names them. Each cross-operation field declares exactly one, the map names it beside the field, and a supplied shape that is not the declared one stops with `input_shape_mismatch` before the operation runs, naming the field, the declared shape, and what arrived. No shape is accepted as a tolerant alternative for another; `workspace_cleanup` accordingly takes the run-owned receipt alone, where it previously also accepted the complete envelope and the creation data.
*Rationale:* `operator_revalidate` reads `captured.data.identity`, so it requires the complete envelope, while the CLI validated only that the field was present. Passing `operator_capture.data` — the plausible object — therefore crossed the boundary and failed later as `capture_failed: complete target identity is required`, an error naming the target rather than the request. That is the observed failure on `tetsuh/sitos#165`: a fresh exact-autofix run stopped before any gate or mutation with 0 gate invocations, 0 pushes, and no repository or GitHub mutation. Fail-closed behaved correctly; the diagnosis pointed at the wrong thing, and because exact autofix never resumes, an incidental composition slip discards a whole run on an unmoved target. Two neighbouring fields showed the same class from the other side: `workspace_cleanup` silently accepted three shapes for one field, and `fingerprint_snapshot` is total over objects, so an envelope supplied there produces a well-formed digest over the wrong bytes rather than an error at all. Rejecting is the whole intervention. This decision adds no recovery, no normalization, and no tolerance: it converts a late misleading failure into an immediate accurate one, and CL-D39 still governs recoverability for the keys it already names. Scope is the packaged CLI boundary because that is where the request is a document rather than a call, and it is where the map the orchestrator follows is authoritative. CL-D42 is complementary and does not overlap: it labels what a value *is* inside the evidence envelope, while this decision declares what shape a *request field* takes. Enforcing the declaration exposed a second naming defect and fixed it: the capture document was stamped `operator_checkout` by the library and restamped `operator_capture` by the CLI, so one document carried two producer names and its own `operation` field could not identify its producer. Every packaged module now stamps the operation the request names, which is what makes an envelope self-identifying; no test depended on the old name.
*Validity and invalidation conditions:* Applies to the packaged CLI request boundary for the declared fields. It grants no authority, consumes no counter, changes no operation's semantics or output, and weakens no fail-closed, no-retry, no-resume, mutation, or publication boundary. Adding a cross-operation field without declaring its shape, accepting a second shape for any field, or normalizing a wrong shape instead of rejecting it requires a new owner decision. If `operator_revalidate` ever stops reading the complete envelope, or `workspace_cleanup` ever needs a shape other than the receipt, the declarations recorded here are invalidated and must be re-derived from the consumers rather than adjusted in place.
