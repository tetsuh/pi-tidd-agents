...
const PR_HELPER_FILES = ['builders.js', 'cli.js', 'composition.js', 'envelope.js', 'evidence.js', 'fingerprints.js', 'gate-result.js', 'guards.js', 'index.js', 'launch.js', 'operator.js', 'paths.js', 'process.js', 'protocol.js', 'reply.js', 'snapshot.js', 'writability.js', 'workspace.js'].map((file) => `${PR_HELPER_DIR}/${file}`);
...
test('Issue #47 packaged helper surface is allowlisted without an entrypoint', () => {
  ...
  assert.ok(helperBytes < 220000, `Issue #59 aggregate helper smoke alarm exceeded: ${helperBytes} bytes`);
...
test('Issue #25 packed artifacts do not require the unpackaged development record', () => {
  ...
  try {
    ...
    assert.equal(files.length, 37, `packed file count changed: ${files.join(', ')}`);
    assert.ok(!files.includes('CONTRACT.md'));
    for (const file of FALSIFICATION_ARTIFACTS) {
