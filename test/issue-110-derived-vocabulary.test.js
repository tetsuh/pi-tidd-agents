...
test('Issue #110 the vocabulary source matches the packaged agents and the envelope schema', () => {
  ...
  for (const gate of VOCAB.gateIdentities) {
    ...
    const envelope = { schemaVersion: 2, correlation, verdict: 'FIX BEFORE MERGE', evidenceRead: [{ source: 'CONTRACT.md', kind: 'file', readCompletely: true }], findings: [finding], confirmations: [], decisions: [], adversarialResults: gate === 'adversarial' ? [{ claim: 'c', searched: 's', outcome: 'counterexample', evidence: 'e', findingId }] : [] };
...
test('Issue #110 the version 1 window and the marker gate vocabulary derive from the source', () => {
  ...
  for (const gate of window.gates) {
    ...
    const envelope = { schemaVersion: 1, correlation, verdict: 'FIX BEFORE MERGE', evidenceRead: [{ source: 'CONTRACT.md', kind: 'file', readCompletely: true }], findings: [finding], confirmations: [], decisions: [], adversarialResults: gate === 'sol' ? [{ claim: 'c', searched: 's', outcome: 'counterexample', evidence: 'e', findingId }] : [] };
...
test('Issue #110 single-surface mutations are named exactly', () => {
  ...
  const rows = ROLES.map((role) => `${code(role.name)} | ${code(role.model)}`), drift = rows.indexOf('`tidd-drift-reviewer` | `gpt-6-sol`');
  ...
  const cases = [
    ...
    ['indented duplicate role row', 'README.md', (text) => text.replace('\n| `tidd-drift-reviewer` | `gpt-6-sol` |', '\n| `tidd-drift-reviewer` | `gpt-6-sol` | duplicate |\n | `tidd-drift-reviewer` | `gpt-6-sol` |'), roleSurfaceGaps, [`README Included agents rows: found ${[...rows.slice(0, drift + 1), rows[drift], ...rows.slice(drift + 1)].join(', ')}; declared ${rows.join(', ')}`]],
    ['unbackticked duplicate role row', 'README.md', (text) => text.replace('\n| `tidd-drift-reviewer` | `gpt-6-sol` |', '\n| tidd-drift-reviewer | gpt-6-sol | duplicate |\n| `tidd-drift-reviewer` | `gpt-6-sol` |'), roleSurfaceGaps, ['README Included agents malformed row: | tidd-drift-reviewer | gpt-6-sol | duplicate |', `README Included agents rows: found ${[...rows.slice(0, drift), 'tidd-drift-reviewer | gpt-6-sol', ...rows.slice(drift)].join(', ')}; declared ${rows.join(', ')}`]],
  ];
