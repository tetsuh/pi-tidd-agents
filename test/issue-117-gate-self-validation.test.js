...
//
// TDD provenance: recorded with `node --test test/issue-117-gate-self-validation.test.js` at RED
// before the prose, the agent sentences, the record, and the clauses existed. That local output is
// not claimed as repository-preserved evidence.
...
test('Issue #117 the Every-gate block states the envelope duties and the self-validation duty', () => {
  ...
  assert.match(block, /every parent-assigned finding appears in `findings` with `origin: assigned`, its exact tuple, and exactly one confirmation; a fresh finding carries no `blockerKey`; an adversarial result links a `findingId` only for a counterexample or unavailable-evidence outcome, and only to an id present in `findings`/);
...
test('Issue #117 CL-D65 records the duty and widens CL-D1 once more', () => {
  ...
  assert.deepEqual(manifest.clauses.filter((clause) => clause.marker === 'CL-D65').map((clause) => clause.id).sort(), ['CL-D65-payload', 'CL-D65-tests']);
});
